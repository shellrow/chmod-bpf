...
use crate::{bpf, daemon, permission, resource, user::{self, group_exists}};
use inquire::Confirm;
...
pub fn install_daemon() {
    ...
    match user::add_group_to_group("admin", bpf::BPF_GROUP) {
        ...
        Err(e) => {
            log::error!("Failed to add group {} to group admin: {}", bpf::BPF_GROUP, e);
            std::process::exit(1);
        },
    }
    match user::add_current_user_to_group(bpf::BPF_GROUP) {
        ...
        Err(e) => {
            log::error!("Failed to add current user to group {}: {}", bpf::BPF_GROUP, e);
            std::process::exit(1);
        },
    }
    ...
    match std::fs::create_dir_all(script_dir_path) {
        Ok(_) => {
            log::info!("Script directory created at {}.", resource::CHMOD_BPF_SCRIPT_DIR_PATH);
        },
        Err(e) => {
            log::error!("Failed to create script directory at {}: {}", resource::CHMOD_BPF_SCRIPT_DIR_PATH, e);
            std::process::exit(1);
        },
    }
    ...
    match std::fs::write(&script_path, resource::CHMOD_BPF_SCRIPT.to_owned()) {
        ...
        Err(e) => {
            log::error!("Failed to save script to {}: {}", resource::CHMOD_BPF_SCRIPT_PATH, e);
            std::process::exit(1);
        },
    }
    ...
    match permission::set_read_execute_permissions_recursive(resource::CHMOD_BPF_SCRIPT_DIR_PATH) {
        ...
        Err(e) => {
            log::error!("Failed to set read and execute permissions for script directory: {}", e);
            std::process::exit(1);
        },
    }
    ...
    match std::fs::write(&plist_path, resource::CHMOD_BPF_PLIST.to_owned()) {
        ...
        Err(e) => {
            log::error!("Failed to save plist to {}: {}", resource::CHMOD_BPF_PLIST_PATH, e);
            std::process::exit(1);
        },
    }
    ...
    match permission::set_read_write_permissions(resource::CHMOD_BPF_PLIST_PATH) {
        ...
        Err(e) => {
            log::error!("Failed to set read and write permissions for plist file: {}", e);
            std::process::exit(1);
        },
    }
